...
| --- | --- | --- |
| 1 | Contracts & module scaffold | Planned |
...
Without shared contracts, upload, worker, and read endpoints invent incompatible shapes. Frontend and tests need stable field names from day one. Scaffolding the module early keeps Phase 4 code out of ad-hoc route files.
...
Transitions: `pending → processing → completed | failed`. Never rewrite a `completed` row in place for a re-run; insert a **new** `resume_analysis` row (Phase 3A rule: many analyses per resume).

### 1.2 Analysis result JSON (OpenAI → API)

Align with `INTEGRATION.md` concepts:
...
### 1.3 HTTP contracts (draft)

| Method | Path | Auth | Purpose |
| --- | --- | --- | --- |
| `POST` | `/api/v1/resumes` | Student | Upload PDF; enqueue analysis |
| `GET` | `/api/v1/resumes` | Student | List own resumes |
| `GET` | `/api/v1/resumes/:id` | Student (owner) | Resume metadata |
| `GET` | `/api/v1/resumes/:id/analysis` | Student (owner) | Latest analysis (+ status) |
| `DELETE` | `/api/v1/resumes/:id` | Student (owner) | Delete resume (+ storage object) |

Upload success: **202 Accepted** with `{ resumeId, analysisId, status: "pending" }`.
...
Wire routes through `src/routes/index.ts` under `/api/v1/resumes`. Controllers stay thin; services own orchestration; repositories remain the only table access path (Phase 3 boundary).

### 1.5 Steps

1. Add TypeScript types for upload response, analysis status, and OpenAI result DTO.
2. Add Zod schemas for multipart metadata and path params.
3. Create empty controller/service/routes exporting a router.
4. Mount router behind `authenticate` + `authorize('STUDENT')`.
5. Document contracts in this file and prepare Swagger stubs (filled in Milestone 8).

## Milestone 1 exit checklist

| Item | Done when |
| --- | --- |
| Status enum documented and typed | `pending \| processing \| completed \| failed` |
| Analysis JSON shape documented | Matches section 1.2 |
| HTTP routes listed | Table 1.3 agreed |
| Module folder exists | `src/modules/resumes/*` scaffold |
| Routes mounted with student auth | Unauthorized → 401; non-student → 403 |
